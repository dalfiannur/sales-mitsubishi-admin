...
import { Testimonial, TestimonialDto } from "../../../typings/Testimonial";
import usePutTestimonial from "../../../composable/usePutTestimonial";
...
const { mutate, onSuccess, isProcessing, onError } = usePutTestimonial();
...
const onImageUpload = (key: "image" | "avatar", info: { file: UploadFileInfo }) => {
...
watch(
  item,
  (val) => {
    if (val.name) data.value.name = val.name;
    if (val.position) data.value.position = val.position;
